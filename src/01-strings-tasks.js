 ...
 */
const extractNameFromTemplate = (val) => val.replace('Hello, ', '').replace('!', '');
...
const getRectangleString = (width, height) => {
  const topRec = `┌${'─'.repeat(width - 2)}┐\n`;
  const midRec = `│${' '.repeat(width - 2)}│\n`;
  const botRec = `└${'─'.repeat(width - 2)}┘\n`;

  switch (height) {
    case 1:
      return topRec;
    case 2:
      return topRec + botRec;
    default:
      return topRec + midRec.repeat(height - 2) + botRec;
  }
};
...
const encodeToRot13 = (str) => {
  ...
  const alphROT13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let result = '';

  for (let i = 0; i < str.length; i += 1) {
    if (str[i].match(/[a-zA-Z]/)) {
      const index = alph.indexOf(str[i]);
      result += alphROT13[index];
    } else {
      result += str[i];
    }
  }
